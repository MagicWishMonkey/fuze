...
class __email_util__:
    ...
    def format(txt):
        def filter(txt):
            ...
            try:
                return txt.translate(table, __email_util__.__punctuation__)
            except:
                return str(txt).translate(table, __email_util__.__punctuation__)
    ...
    def cache(key, *val):
        if len(val) == 0:
            d = __email_util__.__cache__.get(key)
            return d.clone() if d is not None else d

        val = val[0]
        __email_util__.__cache__.set(key, val.clone())
        return val
    ...
    def parse(txt):
        ...
        if len(txt) == 0:
            return None
        email = EmailAddress(txt)
        val = __email_util__.format(txt)
        if val is None:
            return email

        key = val
        cached = __email_util__.cache(key)
        if cached:
            return cached
        ...
        if domain is None or domain.invalid is True:
            return email

        email.username = username
        email.domain = domain
        email.formatted = "%s@%s" % (username, domain.formatted)
        email.invalid = False
        __email_util__.cache(key, email)
        return email
    ...
    def format(txt):
        def filter(txt):
            ...
            try:
                ...
                return str(txt).translate(table, __domain_util__.__punctuation__)
...
class EmailAddress(object):
    ...
    def is_sub_domain(self):
        if self.domain is not None:
            return self.domain.is_sub_domain
        return False
    ...
    @staticmethod
    def parse(txt):
        return __email_util__.parse(txt)

    @staticmethod
    def is_valid(txt):
        o = __email_util__.parse(txt)
        if o:
            if o.invalid is False:
                return True
        return False
